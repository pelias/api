...
query.filter(
  views.max_character_count_layer_filter(
    ['address'],
    config.get('api.autocomplete.exclude_address_length'),
  ),
);
...
function generateQuery(clean) {
  ...
  if (clean.lang && _.isString(clean.lang.iso6391)) {
    ...
    const field = toSingleField(
      vs.var('admin:add_name_lang_to_multimatch:field').get(),
      clean.lang.iso6391,
    );
