...
module.exports = function (adminFields) {
  const subview = peliasQuery.view.admin_multi_match(
    adminFields,
    'peliasQuery',
  );
  ...
  return function (vs) {
    ...
    if (vs.isset('input:street')) {
      return ngrams_last_token_only(vs);
    }
    ...
    if (!complete_tokens || complete_tokens.length < 1) {
      return ngrams_last_token_only(vs);
    }

    // return the simple view when every complete token is numeric
    var all_complete_tokens_numeric = complete_tokens.every(
      (token) => !token.replace(/[0-9]/g, '').length,
    );
    if (all_complete_tokens_numeric) {
      return ngrams_last_token_only(vs);
    }
