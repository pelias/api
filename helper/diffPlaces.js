...
function isLayerDifferent(item1, item2) {
  if (isPropertyDifferent(item1, item2, 'layer')) {
    ...
    if (
      (item1.layer === 'venue' || !_.includes(canonicalLayers, item1.layer)) &&
    ...
    if (
      _.includes(layerPreferences, item1.layer) &&
      _.includes(layerPreferences, item2.layer)
    ) {
      return false;
    }
    return true;
...
function isNameDifferent(item1, item2, requestLanguage) {
  ...
  for (let lang in names2) {
    if (!isPropertyDifferent({ [lang]: names1.default }, names2, lang)) {
      return false;
    }
    if (
      requestLanguage &&
      !isPropertyDifferent({ [lang]: names1[requestLanguage] }, names2, lang)
    ) {
      return false;
    }
  }
  ...
  for (let lang in names1) {
    if (!isPropertyDifferent({ [lang]: names2.default }, names1, lang)) {
      return false;
    }
    if (
      requestLanguage &&
      !isPropertyDifferent({ [lang]: names2[requestLanguage] }, names1, lang)
    ) {
      return false;
    }
  }
...
function isAddressDifferent(item1, item2) {
  ...
  if (isPropertyDifferent(address1, address2, 'number')) {
    return true;
  }
  if (isPropertyDifferent(address1, address2, 'street')) {
    return true;
  }
  ...
  if (_.has(address1, 'zip') && _.has(address2, 'zip')) {
    if (isPropertyDifferent(address1, address2, 'zip')) {
      return true;
    }
  }
...
function isDifferent(item1, item2, requestLanguage) {
  if (isLayerDifferent(item1, item2)) {
    return true;
  }
  if (isParentHierarchyDifferent(item1, item2)) {
    return true;
  }
  if (isNameDifferent(item1, item2, requestLanguage)) {
    return true;
  }
  if (isAddressDifferent(item1, item2)) {
    return true;
  }
  return false;
}
...
function isPropertyDifferent(item1, item2, prop) {
  ...
  if (!_.has(item1, prop) && !_.has(item2, prop)) {
    return false;
  }
  ...
  for (let i = 0; i < prop1.length; i++) {
    ...
    for (let j = 0; j < prop2.length; j++) {
      ...
      if (
        normalizeString(prop1StringValue) === normalizeString(prop2StringValue)
      ) {
        return false;
      }
    }
...
function normalizeString(str) {
  return removeAccents(unicode.normalize(str))
    .toLowerCase()
    .split(/[ ,-]+/)
    .join(' ');
}
