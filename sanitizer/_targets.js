...
const INVALID_NEGATIVE_TARGETS_ERROR =
  'contains positive and negative options in a combination ' +
...
function expandAliases(targets, targetMap) {
  const expanded = targets.reduce(
    (acc, target) => acc.concat(targetMap[target]),
    [],
  );
    ...
    sanitize: function _sanitize(raw, clean) {
      ...
      if (_.isEmpty(targetsString)) {
        messages.errors.push(
          opts.paramName +
            ' parameter cannot be an empty string. Valid options: ' +
            getValidKeys(opts.targetMap),
        );
        return messages;
      }
      ...
      const negative_targets = targets
        .filter((t) => t[0] === '-')
      ...
      positive_targets
        ...
        .filter((target) => !opts.targetMap.hasOwnProperty(target))
        .forEach((target) => {
          messages.errors.push(
            `\'${target}\' is an invalid ${
              opts.paramName
            } parameter. Valid options: ${getValidKeys(opts.targetMap)}`,
          );
        });
      ...
      const effective_positive_targets = expandAliases(
        starting_positive_targets,
        opts.targetMap,
      );
      const effective_negative_targets = expandAliases(
        negative_targets,
        opts.targetMap,
      );

      // the final list of targets is the positive list, with the negative list excluded
      const final_targets = effective_positive_targets.filter(
        (t) => !effective_negative_targets.includes(t),
      );
