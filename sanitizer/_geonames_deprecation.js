...
const coarse_reverse_message =
  'coarse /reverse does not support geonames. See https://github.com/pelias/pelias/issues/675 for more info';
...
function _sanitize(raw, clean, opts) {
  ...
  const is_coarse_reverse =
    !_.isEmpty(clean.layers) &&
  ...
  if (
    _.isEqual(clean.sources, ['geonames']) ||
    _.isEqual(clean.sources, ['gn'])
  ) {
    messages.errors.push(coarse_reverse_message);
  } else if (
    _.includes(clean.sources, 'geonames') ||
    _.includes(clean.sources, 'gn')
  ) {
