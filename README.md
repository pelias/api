...
|`enable_address_layer_filter` | no | true | As a performance optimization, this optional filter excludes results from the 'address' layer for single-word inputs. See the [code comments](https://github.com/pelias/api/blob/master/sanitizer/_address_layer_filter.js) for additional detail. Setting this to `false` will enable returning results from the 'address' layer for single word inputs. Be aware that for large indices (1M+ records) the performance of queries such as 'S' or 'Calle' will likely be very slow due to the large number of hits for those common terms. |
