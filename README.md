...
This is the API server for the Pelias project. It's the service that runs to process user HTTP requests and return results as GeoJSON by querying Search Client(Elasticsearch or OpenSearch) and the other Pelias services.
...
|`targets.auto_discover`|no|true|Should `sources` and `layers` be automatically discovered by querying Search client at process startup. (See more info in the [Custom sources and layers](#custom-sources-and-layers) section below).
|`targets.auto_discover_required`|no|false|If set to `true`, type mapping discovery failures will result in a fatal error. This means a valid connection to a working Search client cluster with a Pelias index is _required_ on startup. Setting this to true can help prevent issues such as incorrect configuration in production environments
...
|`autocomplete.exclude_address_length` | no | 0 | As a performance optimization, this optional filter excludes results from the 'address' layer for any queries where the character length of the 'subject' portion of the parsed_text is equal to or less than this many characters in length. Addresses are usually the bulk of the records in Search client, and searching across all of them for very short text inputs can be slow, with little benefit. Consider setting this to 1 or 2 if you have several million addresses in Pelias. |
|`indexName`|*no*|*pelias*|name of the Search Client index to be used when building queries|
...
|`exposeInternalDebugTools`|*no*|true|Exposes several debugging tools, such as the ability to enable Search Client explain mode, that may come at a performance cost or expose sensitive infrastructure details. Not recommended if the Pelias API is open to the public.
...
A good starting configuration file includes this section (fill in the service and Search Client(Elasticsearch or OpenSearch) hosts as needed):
...
The `auto_discover` functionality sends a request to Search Client in order to automatically discover sources and layers from Search Client when the API server starts-up.

In setups with lots of data (hundreds of millions of records loaded), and low CPU resources, the query sent to Search Client can take several seconds to execute, potentially impacting the performance of other queries hitting Search Client at the same time. The query is cached in Search Client for subsequent requests.
