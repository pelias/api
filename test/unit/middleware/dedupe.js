  ...
  test('filter out duplicates', function (t) {
    ...
    dedupe(req, res, function () {
      t.equal(
        res.data.length,
        expectedCount,
        'results have fewer items than before',
      );
      t.end();
    });
  });
...
module.exports.tests.priority = function (test, common) {
  ...
  test('openaddresses with zip takes priority over openaddresses without zip', function (t) {
    ...
    dedupe(req, res, function () {
      ...
      t.deepEqual(
        res.data[0].source_id,
        '654321',
        'openaddresses result with zip won',
      );
      t.end();
    });
  });
  ...
  test('osm with zip takes priority over openaddresses without zip', function (t) {
    ...
    dedupe(req, res, function () {
      ...
      t.deepEqual(
        res.data[0].source_id,
        '654321',
        'openstreetmap result with zip won',
      );
      t.end();
    });
  });
    ...
    var res = {
      data: [
        ...
        },
        {
          ...
          layer: 'locality',
          name: {
            default: ['Philadelphia', 'Colonial Penn (Brm)', 'Phila', 'Philly'],
          },
          parent: {
            continent: ['North America'],
            continent_id: ['102191575'],
