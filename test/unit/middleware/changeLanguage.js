  ...
  test('translations should be mapped in', (t) => {
    ...
    const service = (req, res, callback) => {
      const response = {
        1: {
          names: {
            'requested language': ['replacement name for layer1'],
            // this should be ignored
            'another language': ['name in another language'],
          },
        },
        2: {
          names: {
        ...
        3: {
          names: {
            'requested language': ['replacement name 1 for layer3'],
          },
        },
        4: {
          names: {
            'requested language': ['replacement name 2 for layer3'],
          },
        },
        10: {
          // has names but not in the requested language
          names: {
            'another language': ['replacement name for layer4'],
          },
        },
        11: {
          // no names
      ...
      data: [
        ...
        // doc with only 1 layer name that will be changed and no default name change
        {
          ...
          parent: {
            layer3_id: ['3', '4'],
            layer3: [
              'original name 1 for layer3',
              'original name 2 for layer3',
            ],
    ...
    controller(req, res, () => {
      t.ok(
        logger.isDebugMessage(
          '[language] [debug] missing translation requested language 10',
        ),
      );
      t.ok(
        logger.isDebugMessage(
          '[language] [debug] missing translation requested language 11',
        ),
      );
      t.ok(
        logger.isDebugMessage(
          '[language] [debug] failed to find translations for 12',
        ),
      );
