  ...
  function controller(req, res, next) {
    ...
    libpostalService(req, (err, response) => {
      if (err) {
        ...
        _.some(
          _.countBy(response, (o) => o.label),
          (count) => count > 1,
        )
      ) {
        logger.warn(
          `discarding libpostal parse of '${req.clean.text}' due to duplicate field assignments`,
        );
        return next();
        ...
        if (
          _.has(req.clean.parsed_text, 'country') &&
          iso3166.isISO2Code(req.clean.parsed_text.country)
        ) {
          req.clean.parsed_text.country = iso3166.convertISO2ToISO3(
            req.clean.parsed_text.country,
          );
        }
...
function patchBuggyResponses(response) {
  ...
  if (response.length === 1) {
    ...
    RECAST_LABELS.forEach((recast) => {
      if (!_.has(recast, 'label') || !_.has(recast.label, 'to')) {
        return;
      }
      if (recast.value !== first.value) {
        return;
      }
      if (_.has(recast.label, 'from') && recast.label.from !== first.label) {
        return;
      }
  ...
  let idx = {};
  response.forEach(
    (res, pos) => (idx[res.label] = _.assign({ _pos: pos }, res)),
  );
    ...
    if (_.isPlainObject(road) && _.isString(road.value)) {
      ...
      if (_.isPlainObject(next) && _.isString(next.value)) {
        ...
        if (
          ...
          (_.size(road.value) === 2 &&
            DIAGONAL_DIRECTIONALS.includes(road.value.toLowerCase())) ||
          // or where the following label contains the directional
          // eg. { 'label':'road', 'value':'foo st' }, { 'label':'suburb', 'value':'nw' }
          (_.size(next.value) === 2 &&
            DIAGONAL_DIRECTIONALS.includes(next.value.toLowerCase()))
        ) {
  ...
  if (
    _.isPlainObject(house_number) &&
    !_.isPlainObject(unit) &&
    _.isString(house_number.value)
  ) {
