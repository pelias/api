...
function hasLatLon(result) {
  return (
    _.isFinite(_.get(result.geom, 'lat')) &&
    _.isFinite(_.get(result.geom, 'lon'))
  );
}
...
function is4CommaDelimitedNumbers(bbox) {
  return _.defaultTo(bbox, '').split(',').filter(isFiniteNumber).length === 4;
}
...
function getLayersFilter(clean) {
  ...
  if (
    _.isEmpty(_.get(clean, 'layers', [])) ||
    _.has(clean, ['parsed_text', 'street'])
  ) {
    return () => true;
  }
...
function matchesBoundaryCountry(boundaryCountry, hierarchy) {
  return (
    !boundaryCountry ||
    boundaryCountry.some(
      (countryCode) => _.get(hierarchy, 'country.abbr') === countryCode,
    )
  );
}
...
function atLeastOneLineageMatchesBoundaryCountry(boundaryCountry, result) {
  return (
    !result.lineage ||
    result.lineage.some(_.partial(matchesBoundaryCountry, boundaryCountry))
  );
}
...
function getBoundaryCountryFilter(clean, do_geometric_filters_apply) {
  if (do_geometric_filters_apply && _.has(clean, 'boundary.country')) {
    return _.partial(
      atLeastOneLineageMatchesBoundaryCountry,
      clean['boundary.country'],
    );
  }
...
function atLeastOneLineageMatchesBoundaryGid(boundaryGid, result) {
  return (
    !result.lineage ||
    result.lineage.some(_.partial(matchesBoundaryGid, boundaryGid))
  );
}
...
function getBoundaryGidFilter(clean, do_geometric_filters_apply) {
  if (do_geometric_filters_apply && _.has(clean, 'boundary.gid')) {
    return _.partial(
      atLeastOneLineageMatchesBoundaryGid,
      clean['boundary.gid'],
    );
  }
...
function getBoundaryRectangleFilter(clean, do_geometric_filters_apply) {
  ...
  if (
    do_geometric_filters_apply &&
    ['min_lat', 'min_lon', 'max_lat', 'max_lon'].every((f) => {
      return _.has(clean, `boundary.rect.${f}`);
    })
  ) {
    const polygon = [
      {
        latitude: clean['boundary.rect.min_lat'],
        longitude: clean['boundary.rect.min_lon'],
      },
      {
        latitude: clean['boundary.rect.max_lat'],
        longitude: clean['boundary.rect.min_lon'],
      },
      {
        latitude: clean['boundary.rect.max_lat'],
        longitude: clean['boundary.rect.max_lon'],
      },
      {
        latitude: clean['boundary.rect.min_lat'],
        longitude: clean['boundary.rect.max_lon'],
    ...
    const isPointInsidePolygon = _.partialRight(
...
function getBoundaryCircleFilter(clean, do_geometric_filters_apply) {
  ...
  if (
    ...
    const isPointInCircle = _.partialRight(
...
function placetypeHasNameAndId(hierarchyElement) {
  return (
    !_.isEmpty(_.trim(hierarchyElement.name)) &&
...
function synthesizeDocs(boundaryCountry, result) {
  ...
  if (_.conformsTo(result.geom, { bbox: is4CommaDelimitedNumbers })) {
    ...
    logger.error(
      `could not parse bbox for id ${result.id}: ${_.get(result, 'geom.bbox')}`,
    );
  }
  ...
  function controller(req, res, next) {
    ...
    placeholderService(req, (err, results) => {
      ...
      if (err) {
        ...
        const boundaryCountry = do_geometric_filters_apply
          ? _.get(req, ['clean', 'boundary.country'])
          : undefined;
        ...
        res.data = results
          ...
          .filter(
            getBoundaryCountryFilter(req.clean, do_geometric_filters_apply),
          )
          ...
          .filter(
            getBoundaryRectangleFilter(req.clean, do_geometric_filters_apply),
          )
          // filter out results that aren't in the boundary.circle
          .filter(
            getBoundaryCircleFilter(req.clean, do_geometric_filters_apply),
          )
