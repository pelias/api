...
function error_intercepting_service(service, req) {
  return (street_result, next) => {
    service(req, street_result, (err, interpolation_result, metadata) => {
      if (err) {
        logger.error(
          `[middleware:interpolation] ${_.defaultTo(err.message, err)}`,
        );
...
function setup(service, should_execute, interpolationConfiguration) {
  return function controller(req, res, next) {
    ...
    const street_results = _.get(res, 'data', []).filter(
      (result) => result.layer === 'street',
    );
    ...
    async.map(
      street_results,
      error_intercepting_service(service, req),
      (err, interpolation_results) => {
        ...
        interpolation_results.forEach((interpolation_result, idx) => {
          ...
          const internalDebug = interpolationConfiguration
            ? interpolationConfiguration.getQueryDebug(req, source_result)
            : {};
          ...
          // -- address_parts --
          source_result.address_parts.number =
            interpolation_result.properties.number;
        ...
        if (_.has(res, 'data')) {
          res.data = stable(res.data, (a, b) => {
            if (a.layer === 'address' && b.layer !== 'address') {
              return -1;
            }
            if (a.layer !== 'address' && b.layer === 'address') {
              return 1;
