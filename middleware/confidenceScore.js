...
function checkForDealBreakers(clean, hit) {
  ...
  if (check.assigned(clean.parsed_text.state) && clean.parsed_text.state !== hit.parent.region_a) {
    logger.debug('[confidence][deal-breaker]: state !== region_a');
    return true;
  }

  if (check.assigned(clean.parsed_text.postalcode) && check.assigned(hit.address) &&
      clean.parsed_text.postalcode !== hit.address.zip) {
    logger.debug('[confidence][deal-breaker]: postalcode !== zip (' + clean.parsed_text.postalcode + ' !== ' + hit.address.zip + ')');
    return true;
  }
}
